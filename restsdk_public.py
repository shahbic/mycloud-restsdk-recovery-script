...
CopyErr='/location/to/dump/files/to/CopyErr.txt' #refereance of documents which has not been copied due to missing directory or error 
...
for root,dirs,files in os.walk(filedir): #find all files in original directory structure
    for file in files:
        ...
        if fullpath!=None:
            ...
            if not os.path.exists(newpath): #avoid recopy incase script stopped during execution
                print('Copying ' + newpath)
                try:
                    os.makedirs(os.path.dirname(newpath), exist_ok=True)
                    #copyfile(fullpath,newpath) #old
                    shutil.copy2(fullpath, newpath, follow_symlinks=True) #copy files with their meta data, better then using copyfile(fullpath,newpath)
                except:
                    print('Error copying file ' + fullpath + ' to ' + newpath)
                    f = open(CopyErr, "a")
                    f.write(fullpath + ' || ' + newpath + "\n")
                    f.close()
...
#Optional which could be removed. Send an email once the recovery process is done
# Define the SMTP server here:
port = 587 
smtp_server = "mail.yourdomain.com"
login = "name@yourdomain.com" # paste your login generated by Mailtrap
password = "password" # paste your password generated by Mailtrap
sender = "name@yourdomain.com"
receiver = "name@yourdomain.com"
message = f"""\
Subject: Recovery Done
To: {receiver}
From: {sender}
